...
edge_model=load('models/forest/modelBsds'); edge_model=edge_model.model;
...
%cnnweights = './caffe/models/bvlc_alexnet/.caffemodel';
cnnmodel = '/home/leo/docker_ws/models/caffe/bvlc_alexnet/bvlc_alexnet.prototxt';
cnnweights = '/home/leo/docker_ws/models/caffe/bvlc_alexnet/bvlc_alexnet.caffemodel';
...
d = load('/home/leo/docker_ws/models/caffe/bvlc_alexnet/ilsvrc_2012_mean.mat');

net = importCaffeNetwork(cnnmodel,cnnweights);
...
bboxes_2size = [];
for ii = 1:s1
    if ((bboxes(ii,1)+ box_size) < d1) && ((bboxes(ii,2)+box_size) < d2)
        bboxes_2size =[bboxes_2size;bboxes(ii,:)];
    end
end
[ss1, ss2] = size(bboxes_2size);

%bboxes = bboxes_2size(ss1,:);
bboxes = bboxes_2size;

nLandmarks = min(20, size(bboxes,1));
abboxes = bboxes(1:nLandmarks, :);
MatBBox = bboxes;
...
for i = 1:nLandmarks
    ...
    x1=bboxes(i, 1); y1=bboxes(i, 2); x2=bboxes(i, 3); y2=bboxes(i, 4);
    fprintf('x = %s -> %s and y = %s -> %s \n',num2str(x1),num2str(x2),num2str(y1),num2str(y2));
    %fprintf('Box size has h = %s and w = %s \n',num2str(h),num2str(w));
    
    bounding_box = im(x1:x1+box_size-1,y1:y1+box_size-1, :); 
    ...
    res = activations(net,im_data,7);
    
    %feature = res{1}; % get feature 
    feature = convertToVec(res);
