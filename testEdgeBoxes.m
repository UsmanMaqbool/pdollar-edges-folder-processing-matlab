...
edge_model=load('edges-fixed-version/models/forest/modelBsds'); edge_model=edge_model.model;
...
for j = 1:length(images)
    ...
    bboxes, E =edgeBoxes(im,edge_model);
    gt = bboxes;
    ...
    for i = 1:size(gtRes,1)
        ...
        h=abs(box(3)-x )
        w=abs(box(4)-y)
        imwrite(im(x:x+h-1, y:y+w-1, :), ['test' '/' int2str(j) '/' images(j).name '_' int2str(i) '.jpg']);
    end
